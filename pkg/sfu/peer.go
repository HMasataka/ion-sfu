/*
【ファイル概要: peer.go】
このファイルはピア接続の管理を実装します。

【主要な役割】
1. Peerインターフェースとその実装（PeerLocal）
   - 1つのピアは通常2つのWebRTC PeerConnection を持ちます：
     * Publisher接続: ピアからSFUへのメディア送信
     * Subscriber接続: SFUからピアへのメディア送信

2. Pub/Sub モデル
   - 各ピアはパブリッシャーとサブスクライバーの両方の役割を果たします
   - これにより、双方向メディア通信が実現されます
...
4. ライフサイクル管理
   - Join: セッションへの参加とピア接続の確立
   - Close: すべてのリソースのクリーンアップ

【アーキテクチャ】
PeerはPublisherとSubscriberを組み合わせた高レベルの抽象化です。
これにより、アプリケーションコードはWebRTCの複雑さから隔離されます。
*/
package sfu
