/*
【ファイル概要: subscriber.go】
サブスクライバーはSFUからピアへのメディア配信を管理します。

【主要な役割】
1. 送信トラック（DownTrack）の管理
   - 複数のDownTrackを保持（各パブリッシャーごと）
   - トラックの追加・削除
   - ストリームIDによるグループ化
...
3. SDPネゴシエーション
   - Offerの作成（ダウントラック追加時）
   - リモートSDPの設定（Answerの受信）
   - デバウンスされたネゴシエーション（頻繁な変更の集約）

4. RTCP レポート送信
   - Sender Reportの定期的な生成
   - Source Descriptionの送信
   - ダウントラック統計の集約

【ネゴシエーションの最適化】
ネゴシエーションは250msのデバウンスで遅延され、
複数のトラック追加を1回のネゴシエーションにまとめます。
*/
package sfu
